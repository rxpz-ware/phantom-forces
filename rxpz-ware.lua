...
function SaveBullshitSettings()
	...
	local succ, out = pcall(function()
		...
		writefile("rxpz-ware phantom forces script", HTTP:JSONEncode(temp))
	end)
	if not succ then
		error(out)
	end
...
fuck = pcall(function()
	local temp = HTTP:JSONDecode(readfile("Rxpz! ✓#1004"))
...
function InitMain()
	...
	Title.Text = "rxpz-ware phantom forces script\nVersion 3.5.5 (RE-WORK IN THE WORKS)"
	...
	TextLabel.Text = [[
        rxpz-ware offical phantom forces script
]]
